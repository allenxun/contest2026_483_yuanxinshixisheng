...
/**
 * 未配置（缺 base-url/api-key）且非生产时报告播报端点的降级语义（HTTP 级）：
 * 预流 503 DEPENDENCY_UNAVAILABLE，应用仍可启动，绝不回退替身/合成文案。
 *
 * <p>仓库内 {@code app.report-narration.*} 默认空占位；本测试不注入任何真实值，也不联网。</p>
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.MOCK)
@AutoConfigureMockMvc
@ActiveProfiles("test")
class ReportNarrationUnconfiguredEndpointIT {
    ...
    private Seeded seedReportReadyTask() {
        String serial = "SN-RN-UNC-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    @Test
    @DisplayName("未配置（非生产）→ 503 DEPENDENCY_UNAVAILABLE JSON（不启动失败、无下游）")
    void unconfiguredReturns503() throws Exception {
