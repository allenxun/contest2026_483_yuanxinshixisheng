 ...
 * 因此<b>明文验证码绝不落入 Redis</b>（比现状的内存明文更严格：运维/日志/误打印都拿不到明文），
 * 且相同验证码在不同 challenge 下摘要不同，无法建立跨 challenge 的全局对照表。</p>
 *
 * <p><b>如实限制（不得夸大）</b>：验证码只有 6 位十进制（约 20 bit 熵），而 {@code challengeId}
 * 是<b>返回给客户端的</b>、并非秘密。因此持有 Redis <b>只读快照</b>的攻击者可以离线枚举全部
 * 10^6 个候选码并比对摘要——加盐 sha256 <b>不能</b>抵御这种离线枚举。它的实际价值是
 * "明文不出现在 Redis 与任何日志/导出中"，而不是"快照泄漏也安全"。若要真正抵御快照泄漏，
 * 需要以<b>服务端 secret（pepper）</b>做 HMAC，本轮未实现（会引入新的秘密分发与轮换问题），
 * 已如实登记为限制。在线暴力枚举仍由"尝试次数上限 + challenge TTL"原子限制。</p>
 *
