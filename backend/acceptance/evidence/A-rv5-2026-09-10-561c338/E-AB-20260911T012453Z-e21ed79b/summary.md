# E RV-5 有界复验（PARTIAL）——A 561c338137aa，run E-AB-20260911T012453Z-e21ed79b
...
| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| RV5-1 | 从当前源码重建：A 源码==561c338（git diff 空）+ mvn 重建 jar + 启动健康；未用镜像 | **PASS** | git diff 561c338 -- web-java worker-python contracts; mvn -DskipTests package; java -jar / 0 | diff_empty=True HEAD=fb2fc080bc0e jar=web-java-0.0.1-SNAPSHOT.jar sha256[:16]=c12077b9a8f42924 health_up=True；同源各自重建，jar 哈希可能因构建元数据不同而非字节一致 |
...
| RV5-6 | 有界严格响应契约：成功投影走严格 schema（validate_responses）+ selftest 10/10 + samples 50 + OpenAPI；404/401 仅基本信封检查（如实分类） | **PASS** | validate_responses <200 捕获>; --selftest; validate_samples; openapi_spec_validator / 0/1 | selftest=True samples=True(50) oas=True captured_200_strict=True / 404/401=基本信封检查: 3Z-e21ed79b/logs/responses/rv5-6-1.json ok /home/lousuan/lansee/openvela/conte |
