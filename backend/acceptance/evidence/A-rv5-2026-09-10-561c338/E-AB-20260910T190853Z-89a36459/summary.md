# E RV-5 有界复验（PARTIAL）——A 561c338137aa，run E-AB-20260910T190853Z-89a36459
...
| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| RV5-1 | 从当前源码重建：A 源码==561c338（git diff 空）+ mvn 重建 jar + 启动健康；未用镜像 | **PASS** | git diff 561c338 -- web-java worker-python contracts; mvn -DskipTests package; java -jar / 0 | diff_empty=True HEAD=e1d54b92e8b6 jar=web-java-0.0.1-SNAPSHOT.jar sha256[:16]=c12077b9a8f42924 health_up=True；同源各自重建，jar 哈希可能因构建元数据不同而非字节一致 |
...
| RV5-3 | GET 三态统一 404 不可区分：外来/不存在/非 echo，error 子树规范化等值 + 无归属/类型泄露 | **PASS** | 三态 GET + requestId 外 error 子树逐字节比对 / 404/404/404 | accounts_differ=True s1_valid=True s2_valid=True status=404/404/404 error_equal=True code=['RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE'] no_leak=True |
...
| RV5-5 | POST dedup 碰撞矩阵（①无键 404 不投影 ②keyed 404+T13 rejected ③重放同一拒绝 ④A 行不变 ⑤无 B 属行 ⑥同主体正例） | **PASS** | A 显式 body.jobId 建 job → B 同 jobId 无键/keyed 碰撞 + SQL 查证 / 404/404/404/200 | no-key: 404/RESOURCE_NOT_VISIBLE no_projection=True same_err=True | keyed#1: 404 t13=rejected|RESOURCE_NOT_VISIBLE | keyed#2: 404 replay_same_error=True | A_row_unchanged=True | B_owned_rows=0 | positive_replay: 200 same_job=True |
| RV5-6 | 有界严格响应契约：成功投影走严格 schema（validate_responses）+ selftest 10/10 + samples 50 + OpenAPI；404/401 仅基本信封检查（如实分类） | **PASS** | validate_responses <200 捕获>; --selftest; validate_samples; openapi_spec_validator / 0/1 | selftest=True samples=True(200) oas=True captured_200_strict=True | 404/401=基本信封检查: 3Z-89a36459/logs/responses/rv5-6-1.json ok /home/lousuan/lansee/openvela/conte |
