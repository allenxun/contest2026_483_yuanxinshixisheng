# E 独立 A 基线验收证据（2026-09-10，run E-AB-20260910T133648Z-7a35b619）
...
环境：E 专用 PG `mvp-e-pg`@127.0.0.1:55433（label mvp.e.run=E-AB-20260910T133648Z-7a35b619，镜像 postgres:16），Java@18081，worker 健康@18082；94 业务场景保持 dependency_pending（B/C/D 未集成）。
...
| AB-01b | Java dev 启动 + /actuator/health UP（健康绑定子进程存活） | **PASS** | java -jar (SERVER_PORT=18081, db=mvp_e_dev) / 0 | first pid=3448780 |
...
| AB-05a | 无 token → 401 AUTH_REQUIRED 信封 | **PASS** | GET /me/member-access-grants / 401 | {"requestId": "f7d54110-1808-40ae-910a-e9bb8a78cb59", "error": {"code": "AUTH_REQUIRED", "message": "missing bearer token", "retryable": false}} |
| AB-05b | 伪造 Bearer → 401 SESSION_INVALID | **PASS** | GET with forged token / 401 | {"requestId": "df491349-5a0a-4725-83a1-0cdb57edc6f7", "error": {"code": "SESSION_INVALID", "message": "session token invalid or expired", "retryable": false}} |
| AB-05c | 手机号会话建立（SMS 替身 123456 + installationId） | **PASS** | POST sms-challenges→sessions / 200 | accountId=7178de7d-665c-425c-ab98-6f6d918cb952 前置有效码=501 |
| AB-05d | refresh 轮换后旧 refresh token → 401 SESSION_INVALID（独立会话） | **PASS** | POST session-refreshes (old reused) / 200/401 | {"requestId": "11d44b91-fcd1-479c-81b8-b7182c402b5c", "error": {"code": "SESSION_INVALID", "message": "refresh credential invalid or rotated", "retryable": false}} |
| AB-05e | 账号 disabled 后旧 token → 401 SESSION_INVALID（每请求复核） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=7178de7d |
| AB-05f | auth_revision 递增后旧 token → 401（撤销代次不可复活） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=7178de7d |
...
| AB-06a | echo POST → 200 queued + jobId | **PASS** | POST /system/echo-jobs (Idempotency-Key) / 200 | {"requestId": "5c22845f-cf3a-4484-8cc0-ad62e68996d8", "data": {"jobId": "9f68d39f-98a3-42c4-9799-b3eb59ac4b8d", "dedupKey": "system:echo:8657b6d4-a5c4-4475-896a-4f21f85a34ad", "status": "queued"}, "meta": {"replayed": false, "serverTime": "2026-09-10 |
| AB-08a | T13 同 Idempotency-Key 同内容重放 → 同 jobId + meta.replayed=true | **PASS** | POST same key/content / 200 | replayed=True same_job=True |
| AB-08b | T13 同键不同内容 → 409 冲突（非 200/假成功） | **PASS** | POST same key/different content / 409 | {"requestId": "d934f838-5cb4-4a9b-bf0d-bf594930e79f", "error": {"code": "IDEMPOTENCY_CONTENT_CONFLICT", "message": "Idempotency-Key reused with different request content; use a new key for a new logic |
| AB-06b | worker --once 领取并处理本次 enqueued job（日志含 jobId 或状态推进） | **PASS** | python -m mvp_worker --once / 0 | job=9f68d39f status=succeeded |
...
| AB-11a | 非法输入 → 400 INVALID_INPUT 信封，X-Request-Id==body.requestId（均非空） | **PASS** | POST /system/echo-jobs {} / 400 | rid_hdr=54b682a7-c4ba-43f3-89ef-03d96416197a rid_body=54b682a7-c4ba-43f3-89ef-03d96416197a |
| AB-11b | 业务 stub（有 token）→ 501 NOT_IMPLEMENTED，不给假 200 | **PASS** | GET /me/member-access-grants / 501 | {"requestId": "1f226c77-01bd-43e8-a479-fa1e934c585a", "error": {"code": "NOT_IMPLEMENTED", "message": "business endpoint is contract-only and not implemented yet", "retryable": false, "details": {"api |
| AB-11c | 未知资源 → 404 结构化信封（含 requestId） | **PASS** | GET /system/echo-jobs/<random> / 404 | {"requestId": "5149eea2-6887-4ba7-8e85-00882ea7354b", "error": {"code": "RESOURCE_NOT_VISIBLE", "message": "job not visible", "retryable": false}} |
...
| N3-media | 媒体 deny-all：任何 GET 统一 404（含 face purpose、含真实上传者本人、无 403 泄露） | **PASS** | 真实上传者主体 GET /media/{id}/content / 404/404 | uploader_self_404=True no_403=True uploader=7178de7d |
...
| N1-py-runtime | Python 运行时拒绝非法 schema_version（failed/UNSUPPORTED_CONTRACT，不循环） | **PASS** | worker --once 处理坏版本 job / 0/1 | 293d95:failed/UNSUPPORTED_CONTRACT 293d95:failed/UNSUPPORTED_CONTRACT 293d95:failed/UNSUPPORTED_CONTRACT |
...
- 证据日志：reports/E-AB-20260910T133648Z-7a35b619/ 与 logs/ 摘录（非空）。
