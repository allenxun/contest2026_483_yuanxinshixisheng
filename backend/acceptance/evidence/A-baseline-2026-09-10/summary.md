# E 独立 A 基线验收证据（2026-09-10，run E-AB-20260910T130438Z）
...
环境：E 专用 PG `mvp-e-pg`@127.0.0.1:55433（label mvp.e.run=E-AB-20260910T130438Z，镜像 postgres:16），Java@18081，worker 健康@18082；94 业务场景保持 dependency_pending（B/C/D 未集成）。
...
## 结算：52/52 唯一结算；51 PASS / 1 FAIL / 0 BLOCKED；final_exit=1

结算缺口：missing=[] extra=[] duplicates=[]
...
| AB-01b | Java dev 启动 + /actuator/health UP（健康绑定子进程存活） | **PASS** | java -jar (SERVER_PORT=18081, db=mvp_e_dev) / 0 | first pid=3390853 |
...
| AB-02c | worker SIGTERM 优雅停机 | **PASS** | kill -TERM worker / 0 |  |
| AB-02d | E 定向执行 worker pytest（claim/renew/expire/complete/attempt-ceiling/unsupported；E 专属 PG 临时库，绝不混 A 库） | **PASS** | pytest -q tests/test_claim.py tests/test_renew.py tests/test_expire.py tests/test_complete.py tests/test_attempt_ceiling.py tests/test_unsupported.py / 0 | passed=26 : 26 passed in 5.36s |
...
| AB-05a | 无 token → 401 AUTH_REQUIRED 信封 | **PASS** | GET /me/member-access-grants / 401 | {"requestId": "5f6af176-2216-4107-9277-9c10cee82ae0", "error": {"code": "AUTH_REQUIRED", "message": "missing bearer token", "retryable": false}} |
| AB-05b | 伪造 Bearer → 401 SESSION_INVALID | **PASS** | GET with forged token / 401 | {"requestId": "ce530b6f-0b40-4b38-99fb-145ed8b714c2", "error": {"code": "SESSION_INVALID", "message": "session token invalid or expired", "retryable": false}} |
| AB-05c | 手机号会话建立（SMS 替身 123456 + installationId） | **PASS** | POST sms-challenges→sessions / 200 | accountId=8c72a73d-3b07-43e5-8491-d2f22c9ceabf 前置有效码=501 |
| AB-05d | refresh 轮换后旧 refresh token → 401 SESSION_INVALID（独立会话） | **PASS** | POST session-refreshes (old reused) / 200/401 | {"requestId": "8330b7e2-1af1-476c-a41a-0f525ef55e43", "error": {"code": "SESSION_INVALID", "message": "refresh credential invalid or rotated", "retryable": false}} |
| AB-05e | 账号 disabled 后旧 token → 401 SESSION_INVALID（每请求复核） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=8c72a73d |
| AB-05f | auth_revision 递增后旧 token → 401（撤销代次不可复活） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=8c72a73d |
...
| AB-06a | echo POST → 200 queued + jobId | **PASS** | POST /system/echo-jobs (Idempotency-Key) / 200 | {"requestId": "ca6ddce3-714b-412c-9097-d6fb55fe9763", "data": {"jobId": "ed673579-4541-4363-bc72-5bbc30e97b4c", "dedupKey": "system:echo:79349b10-4490-4e0b-babe-93f76676545b", "status": "queued"}, "meta": {"replayed": false, "serverTime": "2026-09-10 |
| AB-08a | T13 同 Idempotency-Key 同内容重放 → 同 jobId + meta.replayed=true | **PASS** | POST same key/content / 200 | replayed=True same_job=True |
| AB-08b | T13 同键不同内容 → 409 冲突（非 200/假成功） | **PASS** | POST same key/different content / 409 | {"requestId": "943d05f6-1111-43f7-b8e7-28c5adbf73d2", "error": {"code": "IDEMPOTENCY_CONTENT_CONFLICT", "message": "Idempotency-Key reused with different request content; use a new key for a new logic |
| AB-06b | worker --once 领取并处理本次 enqueued job（日志含 jobId 或状态推进） | **PASS** | python -m mvp_worker --once / 0 | job=ed673579 status=succeeded |
...
| AB-11a | 非法输入 → 400 INVALID_INPUT 信封，body.requestId==X-Request-Id | **PASS** | POST /system/echo-jobs {} / 400 | rid_hdr=4d8e4262-955d-4c6a-a582-f9c98c1f120d rid_body=4d8e4262-955d-4c6a-a582-f9c98c1f120d |
| AB-11b | 业务 stub（有 token）→ 501 NOT_IMPLEMENTED，不给假 200 | **PASS** | GET /me/member-access-grants / 501 | {"requestId": "01b577db-9659-4bf0-bb9b-6cb6ff632b16", "error": {"code": "NOT_IMPLEMENTED", "message": "business endpoint is contract-only and not implemented yet", "retryable": false, "details": {"api |
| AB-11c | 未知资源 → 404 结构化信封（含 requestId） | **PASS** | GET /system/echo-jobs/<random> / 404 | {"requestId": "42067332-f397-46fe-a420-da3ed5b82b40", "error": {"code": "RESOURCE_NOT_VISIBLE", "message": "job not visible", "retryable": false}} |
| N2-http | 诊断列 HTTP 投影：须脱敏/限大小/不原样返回（实测 data.lastError） | **FAIL** | GET /system/echo-jobs/{id} 观察 data.lastError / 200/200 | marker_echoed=True raw_lastError_len=4058 raw_projected=True；A 缺陷最小复现见 logs/n2-http-repro.txt |
| N3-media | 媒体 deny-all：任何 GET 统一 404（含 face purpose、含真实上传者本人、无 403 泄露） | **PASS** | 真实上传者主体 GET /media/{id}/content / 404/404 | uploader_self_404=True no_403=True uploader=8c72a73d |
...
| N1-py-runtime | Python 运行时拒绝非法 schema_version（failed/UNSUPPORTED_CONTRACT，不循环） | **PASS** | worker --once 处理坏版本 job / 0/1 | 7f900c:failed/UNSUPPORTED_CONTRACT 7f900c:failed/UNSUPPORTED_CONTRACT 7f900c:failed/UNSUPPORTED_CONTRACT |
...
| N2-codereview | 诊断列消费点分类检视（23 处，客户端投影/写入/人工检视） | **PASS** | grep Java/Python src + 分类记录 / 0 | hits=23 详见 logs/n2-code-review.md |
...
- **N2-http** 诊断列 HTTP 投影：须脱敏/限大小/不原样返回（实测 data.lastError）；命令=GET /system/echo-jobs/{id} 观察 data.lastError；rc=200/200；摘录=marker_echoed=True raw_lastError_len=4058 raw_projected=True；A 缺陷最小复现见 logs/n2-http-repro.txt
...
- 证据日志：reports/E-AB-20260910T130438Z/ 与 logs/ 摘录（非空）。
