# E 独立 A 基线验收证据（2026-09-10，run E-AB-20260910T130740Z）
...
环境：E 专用 PG `mvp-e-pg`@127.0.0.1:55433（label mvp.e.run=E-AB-20260910T130740Z，镜像 postgres:16），Java@18081，worker 健康@18082；94 业务场景保持 dependency_pending（B/C/D 未集成）。
...
| AB-01b | Java dev 启动 + /actuator/health UP（健康绑定子进程存活） | **PASS** | java -jar (SERVER_PORT=18081, db=mvp_e_dev) / 0 | first pid=3396297 |
...
| AB-02d | E 定向执行 worker pytest（claim/renew/expire/complete/attempt-ceiling/unsupported；E 专属 PG 临时库，绝不混 A 库） | **PASS** | pytest -q tests/test_claim.py tests/test_renew.py tests/test_expire.py tests/test_complete.py tests/test_attempt_ceiling.py tests/test_unsupported.py / 0 | passed=26 : 26 passed in 5.55s |
...
| AB-05a | 无 token → 401 AUTH_REQUIRED 信封 | **PASS** | GET /me/member-access-grants / 401 | {"requestId": "91419e92-3c93-4866-b7ea-26d3bab59cb2", "error": {"code": "AUTH_REQUIRED", "message": "missing bearer token", "retryable": false}} |
| AB-05b | 伪造 Bearer → 401 SESSION_INVALID | **PASS** | GET with forged token / 401 | {"requestId": "224846dc-9295-4955-92cf-cc76a3356117", "error": {"code": "SESSION_INVALID", "message": "session token invalid or expired", "retryable": false}} |
| AB-05c | 手机号会话建立（SMS 替身 123456 + installationId） | **PASS** | POST sms-challenges→sessions / 200 | accountId=883c6d83-d789-4363-82e4-bc5847578dc2 前置有效码=501 |
| AB-05d | refresh 轮换后旧 refresh token → 401 SESSION_INVALID（独立会话） | **PASS** | POST session-refreshes (old reused) / 200/401 | {"requestId": "f89b5add-7511-4fce-9a53-3d055363e4db", "error": {"code": "SESSION_INVALID", "message": "refresh credential invalid or rotated", "retryable": false}} |
| AB-05e | 账号 disabled 后旧 token → 401 SESSION_INVALID（每请求复核） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=883c6d83 |
| AB-05f | auth_revision 递增后旧 token → 401（撤销代次不可复活） | **PASS** | 独立会话 → 变更真实 accountId → GET / 501->401 | rows_changed=1 account=883c6d83 |
...
| AB-06a | echo POST → 200 queued + jobId | **PASS** | POST /system/echo-jobs (Idempotency-Key) / 200 | {"requestId": "bf91a0d1-4dbf-452b-aca9-b407a22d852f", "data": {"jobId": "8d6e71b7-f8e2-4934-98a2-d33dc265379f", "dedupKey": "system:echo:781f7d07-fafb-45c1-8230-5afa4702b22d", "status": "queued"}, "meta": {"replayed": false, "serverTime": "2026-09-10 |
| AB-08a | T13 同 Idempotency-Key 同内容重放 → 同 jobId + meta.replayed=true | **PASS** | POST same key/content / 200 | replayed=True same_job=True |
| AB-08b | T13 同键不同内容 → 409 冲突（非 200/假成功） | **PASS** | POST same key/different content / 409 | {"requestId": "60bf2f7b-1359-4318-9f76-f44d63e4e0f9", "error": {"code": "IDEMPOTENCY_CONTENT_CONFLICT", "message": "Idempotency-Key reused with different request content; use a new key for a new logic |
| AB-06b | worker --once 领取并处理本次 enqueued job（日志含 jobId 或状态推进） | **PASS** | python -m mvp_worker --once / 0 | job=8d6e71b7 status=succeeded |
...
| AB-11a | 非法输入 → 400 INVALID_INPUT 信封，body.requestId==X-Request-Id | **PASS** | POST /system/echo-jobs {} / 400 | rid_hdr=f4faa3ec-e149-444e-bcc8-9b99eb9bad10 rid_body=f4faa3ec-e149-444e-bcc8-9b99eb9bad10 |
| AB-11b | 业务 stub（有 token）→ 501 NOT_IMPLEMENTED，不给假 200 | **PASS** | GET /me/member-access-grants / 501 | {"requestId": "6f59d4eb-b131-42bb-8bc5-16c841ad1f31", "error": {"code": "NOT_IMPLEMENTED", "message": "business endpoint is contract-only and not implemented yet", "retryable": false, "details": {"api |
| AB-11c | 未知资源 → 404 结构化信封（含 requestId） | **PASS** | GET /system/echo-jobs/<random> / 404 | {"requestId": "8f6c9646-5701-4a71-a031-64a3a87ccbee", "error": {"code": "RESOURCE_NOT_VISIBLE", "message": "job not visible", "retryable": false}} |
...
| N3-media | 媒体 deny-all：任何 GET 统一 404（含 face purpose、含真实上传者本人、无 403 泄露） | **PASS** | 真实上传者主体 GET /media/{id}/content / 404/404 | uploader_self_404=True no_403=True uploader=883c6d83 |
...
| N1-py-runtime | Python 运行时拒绝非法 schema_version（failed/UNSUPPORTED_CONTRACT，不循环） | **PASS** | worker --once 处理坏版本 job / 0/1 | 0901c1:failed/UNSUPPORTED_CONTRACT 0901c1:failed/UNSUPPORTED_CONTRACT 0901c1:failed/UNSUPPORTED_CONTRACT |
...
- 证据日志：reports/E-AB-20260910T130740Z/ 与 logs/ 摘录（非空）。
