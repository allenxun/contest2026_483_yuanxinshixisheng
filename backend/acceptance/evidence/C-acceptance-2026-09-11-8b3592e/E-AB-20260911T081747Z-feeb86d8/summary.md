# E C/M4 黑盒验收 —— C 8b3592e，run E-AB-20260911T081747Z-feeb86d8
...
## 结算：14/14 唯一结算；12 PASS / 2 FAIL / 0 BLOCKED / 0 INFO（计数和=14==行数 14）；final_exit=1

**C/M4 黑盒验收结果：未通过**——12 PASS / 2 FAIL / 0 BLOCKED / 0 INFO。
...
| CC-09 | 账本去重/K/事务：双键判重 duplicate、K 达 completed、同键异内容整批 409 零持久化 | **PASS** | A05 多批 + SQL 核对 / 200/409/409 | accepted=2 disp=[] completed_at_stable=True conflict_zero_rows=0 |
...
- **CC-03** 生产人脸 fail-closed：prod / prod,dev / dev+APP_ENV=production / 非法绑定 拒绝启动；dev 合法绑定正例 A03 201（判别力）；dev+bound positive(attempt 1): A03=503 | dev+bound positive(attempt 2): A03=503 | prod-only: rc=1 refused=True | prod,dev+bound: rc=1 refused=True | dev+APP_ENV=production: rc=1 refused=True | dev+invalid-bound: rc=1 refused=True
