# E C/M4 黑盒验收 —— C 8b3592e，run E-AB-20260912T092138Z-ee4aad22
...
| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| CC-01 | 构建与启动绑定：8b3592e 祖先、**C care 域（care main/test）diff=0**（C 代码未被 B/D/Swagger merge 改动）、当前源码构建、health UP@18081 | **PASS** | git merge-base/diff（care 代码精确路径）; mvn package; java -jar / 0 | HEAD=244b895cc093 ancestor=True care_code_diff=[] contracts_diff=[' backend/contracts/decisions-notes.md / 27 +++++++++++--------', ' backend/contracts/openapi/openapi.yaml / 49 +++++++++++++++++++++-------------', ' 2 files changed, 46 insertions(+), 30 deletions(-)'] jar=web-java-0.0.1-SNAPSHOT.jar sha16=0cceda056b8534d7 |
...
| CC-07 | 幂等与重放：缺键 4xx、同键重放 replayed 且 **verification_revision+last_verified_at 均不刷新**、同键异内容 409 | **PASS** | A03 重放/冲突 / 201/200/409 | nokey=400 same_exec=True replayed=True rev+last_verified 1/2026-09-12 09:22:10.387837+00->1/2026-09-12 09:22:10.387837+00 conflict=True reason=IDEMPOTENCY_CONTENT_CONFLICT |
| CC-08 | 占用/并发：APP+云台同微晶恰一 201 一 409 DEVICE_OCCUPIED；占用仅 closed 释放（closed 前 409 / closed 后 201）；旧任务 A03/A08 → 409 TASK_REPLACED | **PASS** | 并发 + 启停 + SQL 移动 T03 指针（test_seed） / [201, 409] | app/gimbal=409/201 code409=DEVICE_OCCUPIED occ_before=True closed_release=True occ_after=True replaced_a03=True replaced_a08=True(TASK_REPLACED) |
...
| CC-11 | 有界严格契约：selftest 10/10 + samples 50 + OpenAPI VALID；9 个 M4 API（A01-A09）按**实际 HTTP 状态**对应 schema 严格校验（RV-6 转换器，不改 OAS 语义、不展平 allOf）；契约建模问题如实披露、绝不静默放宽 | **PASS** | 契约脚本 + 真实生命周期捕获 9 响应（实际状态）严格校验 / 0/0/0 | selftest=True samples=50 oas_ok=True captured=9 actual_status={'A03': 201, 'A05': 200, 'A04': 200, 'A06': 200, 'A01': 200, 'A02': 200, 'A07': 200, 'A08': 200, 'A09': 200} status_bad=[] impl_bad=[] contract_issues=[] |
...
| CLEANUP | 停止 E 容器（docker stop，保留卷） | **PASS** | docker stop / 0 |  |
