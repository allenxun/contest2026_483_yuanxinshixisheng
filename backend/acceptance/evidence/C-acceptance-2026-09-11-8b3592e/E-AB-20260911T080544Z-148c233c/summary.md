# E C/M4 黑盒验收 —— C 8b3592e，run E-AB-20260911T080544Z-148c233c
...
## 结算：14/14 唯一结算；11 PASS / 3 FAIL / 0 BLOCKED / 0 INFO（计数和=14==行数 14）；final_exit=1

**C/M4 黑盒验收结果：未通过**——11 PASS / 3 FAIL / 0 BLOCKED / 0 INFO。
...
| CC-02 | 权限/统一 404 三态：5 端点陌生/不存在/撤销全等 404 + 云台 403 + 未认证 401 | **FAIL** | 多账号 GET 交叉 + 撤销 grant / 404=[404, 404, 404, 404, 404, 404] | public_equal=False codes=['RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE'] gimbal=403/CALLER_NOT_ALLOWED noauth=401 revoked=404 other=200 seed_ok=True canon_diff={"error": ["{\"code\": \"RESOURCE_NOT_VISIBLE\", \"message\": \"care resource not visible\", \"retryable\": false}", "{\"code\": \"RESOURCE_NOT |
...
| CC-10 | 收尾对账：未 stopped 409 STOP_NOT_CONFIRMED；stopped+水位完整 → closed+occupancyReleased；重放 manifest 不变；A07/A08/A09 2xx | **FAIL** | A06 前后置 + A07/A08/A09 / 409/409 | closed=None released=None accepted=2 manifest_len=0 clo_err=CLOSURE_GAPS/{'reason': 'gaps', 'missingRanges': [{'from': '3', 'to': '3'}], 'more': False} A07=200 A08=200 A09=200 replay=409 |
...
| CC-03 | 生产人脸 fail-closed（prod / prod,dev / dev+APP_ENV=production / 非法绑定）均拒绝启动；dev 合法绑定正例 A03 201（判别力） | **FAIL** | 逐变体串行启动 JVM（限堆 640m）+ A03 正例 / 0 | prod-only: rc=1 refused=True 2026-09-11T16:06:32.897Z level=INFO requestId= logger=o.s.b.a.l.ConditionEvaluationReportLogger msg= Error starting ApplicationContext. To display the condition evaluation report re-run your application with 'debug' enabled. 2026-09-11T16:06:32.921Z level=ERROR requestId= logger=o.s.b.d.LoggingFailureAnalysisReporter msg= *************************** APPLICATION FAILED  |
...
- **CC-02** 权限/统一 404 三态：5 端点陌生/不存在/撤销全等 404 + 云台 403 + 未认证 401；public_equal=False codes=['RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE'] gimbal=403/CALLER_NOT_ALLOWED noauth=401 revoked=404 other=200 seed_ok=True canon_diff={"error": ["{\"code\": \"RESOURCE_NOT_VISIBLE\", \"message\": \"care resource not visible\", \"retryable\": false}", "{\"code\": \"RESOURCE_NOT
- **CC-10** 收尾对账：未 stopped 409 STOP_NOT_CONFIRMED；stopped+水位完整 → closed+occupancyReleased；重放 manifest 不变；A07/A08/A09 2xx；closed=None released=None accepted=2 manifest_len=0 clo_err=CLOSURE_GAPS/{'reason': 'gaps', 'missingRanges': [{'from': '3', 'to': '3'}], 'more': False} A07=200 A08=200 A09=200 replay=409
- **CC-03** 生产人脸 fail-closed（prod / prod,dev / dev+APP_ENV=production / 非法绑定）均拒绝启动；dev 合法绑定正例 A03 201（判别力）；prod-only: rc=1 refused=True 2026-09-11T16:06:32.897Z level=INFO requestId= logger=o.s.b.a.l.ConditionEvaluationReportLogger msg= Error starting ApplicationContext. To display the condition evaluation report re-run your application with 'debug' enabled. 2026-09-11T16:06:32.921Z level=ERROR requestId= logger=o.s.b.d.LoggingFailureAnalysisReporter msg= *************************** APPLICATION FAILED 
