# E C/M4 黑盒验收 —— C 8b3592e，run E-AB-20260911T081344Z-31fa98a8
...
| CC-09 | 账本去重/K/事务：双键判重 duplicate、K 达 completed、同键异内容整批 409 零持久化 | **FAIL** | A05 多批 + SQL 核对 / 200/200/409 | accepted=2 disp=[] completed_at_stable=True conflict_zero_rows=1 |
...
| CC-03 | 生产人脸 fail-closed：prod / prod,dev / dev+APP_ENV=production / 非法绑定 拒绝启动；dev 合法绑定正例 A03 201（判别力） | **FAIL** | 逐变体串行启动 JVM（限堆 640m）+ 正例先行 / 1,1,1,1 | dev+bound positive: A03=503 / prod-only: rc=1 refused=True / prod,dev+bound: rc=1 refused=True / dev+APP_ENV=production: rc=1 refused=True / dev+invalid-bound: rc=1 refused=True |
...
- **CC-02** 权限/统一 404 三态：A01/A02/A07/A08/A09+不存在 全等 404（完整公开体仅排除 requestId）+ 云台 403 + 未认证 401 + 撤销即时生效；public_equal=True codes=['RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE', 'RESOURCE_NOT_VISIBLE'] gimbal=403/CALLER_NOT_ALLOWED noauth=401 revoked=404 other=200 seed_ok=True canon_diff={}
- **CC-09** 账本去重/K/事务：双键判重 duplicate、K 达 completed、同键异内容整批 409 零持久化；accepted=2 disp=[] completed_at_stable=True conflict_zero_rows=1
- **CC-03** 生产人脸 fail-closed：prod / prod,dev / dev+APP_ENV=production / 非法绑定 拒绝启动；dev 合法绑定正例 A03 201（判别力）；dev+bound positive: A03=503 | prod-only: rc=1 refused=True | prod,dev+bound: rc=1 refused=True | dev+APP_ENV=production: rc=1 refused=True | dev+invalid-bound: rc=1 refused=True
