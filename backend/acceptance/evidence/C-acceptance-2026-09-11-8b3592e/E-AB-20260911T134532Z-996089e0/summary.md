# E C/M4 黑盒验收 —— C 8b3592e，run E-AB-20260911T134532Z-996089e0
...
## 结算：14/14 唯一结算；12 PASS / 1 FAIL / 0 BLOCKED / 1 INFO（计数和=14==行数 14）；final_exit=1

**C/M4 黑盒验收结果：未通过**——12 PASS / 1 FAIL / 0 BLOCKED / 1 INFO。

| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| CC-01 | 构建与启动绑定：8b3592e 祖先、**care 域（care main/test + contracts）diff=0**、当前源码构建、health UP@18081 | **FAIL** | git merge-base/diff（care 域精确路径）; mvn package; java -jar / 0 | HEAD=08404f8fbae0 ancestor=True care_domain_diff=['diff --git a/backend/contracts/openapi/openapi.yaml b/backend/contracts/openapi/openapi.yaml', 'index e276926..9af6a72 100644', '--- a/backend/contracts/openapi/openapi.yaml', '+++ b/backend/contracts/openapi/openapi.yaml', '@@ -86,7 +86,7 @@ paths:', ' summary: M1-A01 人脸授权查看本人资料', ' x-api-id: M1-A01', ' x-implementation: contract-only', '- x-erro |
...
| CC-07 | 幂等与重放：缺键 4xx、同键重放 replayed 且 **verification_revision+last_verified_at 均不刷新**、同键异内容 409 | **PASS** | A03 重放/冲突 / 201/200/409 | nokey=400 same_exec=True replayed=True rev+last_verified 1/2026-09-11 13:45:56.944097+00->1/2026-09-11 13:45:56.944097+00 conflict=True reason=IDEMPOTENCY_CONTENT_CONFLICT |
...
- **CC-01** 构建与启动绑定：8b3592e 祖先、**care 域（care main/test + contracts）diff=0**、当前源码构建、health UP@18081；HEAD=08404f8fbae0 ancestor=True care_domain_diff=['diff --git a/backend/contracts/openapi/openapi.yaml b/backend/contracts/openapi/openapi.yaml', 'index e276926..9af6a72 100644', '--- a/backend/contracts/openapi/openapi.yaml', '+++ b/backend/contracts/openapi/openapi.yaml', '@@ -86,7 +86,7 @@ paths:', ' summary: M1-A01 人脸授权查看本人资料', ' x-api-id: M1-A01', ' x-implementation: contract-only', '- x-erro
