# E 定向复验（PARTIAL）——A f6e500e，run E-AB-20260910T153807Z-2fc691d7
...
| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| RV-1 | 从当前源码重建：A 源码==f6e500e（git diff 空）+ 构建 jar + 启动健康 | **PASS** | git diff f6e500e -- web-java worker-python contracts; mvn -DskipTests package; java -jar / 0 | diff_empty=True HEAD=baa809b2ac0a jar=web-java-0.0.1-SNAPSHOT.jar sha256[:16]=25d33dd6fab14461 health_up=True；同源各自重建，jar 哈希可能因构建元数据不同而非字节一致（不作可复现构建声明） |
