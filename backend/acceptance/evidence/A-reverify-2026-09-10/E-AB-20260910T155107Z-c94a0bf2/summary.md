# E 定向复验（PARTIAL）——A f6e500e，run E-AB-20260910T155107Z-c94a0bf2
...
| 项 | 检查 | 状态 | 命令/rc | 关键摘录 |
|---|---|---|---|---|
| RV-1 | 从当前源码重建：A 源码==f6e500e（git diff 空）+ 构建 jar + 启动健康 | **PASS** | git diff f6e500e -- web-java worker-python contracts; mvn -DskipTests package; java -jar / 0 | diff_empty=True HEAD=2a595cfb1a22 jar=web-java-0.0.1-SNAPSHOT.jar sha256[:16]=25d33dd6fab14461 health_up=True；同源各自重建，jar 哈希可能因构建元数据不同而非字节一致（不作可复现构建声明） |
...
| RV-5 | 认证/归属边界：无 token/伪造 401 通过；真跨账号读取的可见性政策待总协调裁定 → INFO（附条件） | **INFO** | GET echo job：no-token/forged/真第二账号（独立身份） / 401/401/200 | no-token: 401/AUTH_REQUIRED | forged: 401/SESSION_INVALID | accounts_differ=True s1_valid=True s2_valid=True other=jobId 匹配; lastError=None || 跨账号可读（200+严格投影）；契约未要求创建者过滤，可见性政策待总协调书面裁定 → INFO |
