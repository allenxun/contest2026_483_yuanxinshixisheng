# E C+D 集成链路验收 —— merged aebccc7（C 8b3592e + D dc955c0），run E-CD-20260911T095047Z-89be0a32
...
| CD-03 | 报告→方案真实链（冻结字段互通）：B 前置 test_seed（gimbal/T04 设备能力）→ 真实 M3-A01 受理 → worker analyze/enroll/analyze → T06 由 D 发布事务唯一创建（waiting_inputs、generation_revision=0、input_photo_version=1、assessment 唯一）→ plan.generate → ready：冻结 capability/steps 形状完整、D 未写 K 列 | **PASS** | M3-A01 + 真实 worker 全链 / a01=202 ready=ready | flags[wait,rev,ipv,dup,ready,cap,steps,K]=True,True,True,True,True,True,True,True task=4c891bde-ff33-4a7b-968c-c60969a6ebed plan=bbd36396-8e6e-4ba8-b6f6-8e60f96de7aa waiting_seen=waiting_inputs cap={"n_bounds": {"max": 100, "min": 1}, "capability_id": "mvp-double-capability", "microcrystal_id": "ed397166-4f05-4409-b94a-953c7bf8f193", "approved_regions": ["forehead", "left_cheek", "right_cheek", st |
...
| CD-05 | 任务替换 vs 准入竞争（真实链路）：真实 M3-A01 受理触发 T03 指针原子替换（非 SQL 种子）；替换前云台 A03=201、替换后旧任务 A03=409 TASK_REPLACED；A08 面：真实链路旧执行已 closed→404（生命周期冻结先于指针），另以 test_seed 指针移动在 admitted 执行上验证 A08=409 TASK_REPLACED；时序化窗口一致（无悬挂/无二者皆成）；CC-08 种子化 TASK_REPLACED 保留边界单测 | **PASS** | M3-A01 + C A03/A08 时序竞争 / 201/409 | pre201=True(ex=fdfc76fb-4077-40c0-a620-9e9f8dd0f1e9) replaced=True(a2=cd16b373-4ca5-4131-b358-f17e0229f383) post_a03=409/TASK_REPLACED a08_real=404/RESOURCE_NOT_VISIBLE a08_boundary=409/TASK_REPLACED consistent=True |
| CD-06 | D 公共 success/failure/defer 回归（黑盒）：success=全链 job succeeded 且 worker 日志 stale_generation=0；defer=plan.generate 能力等待跳（T12 queued、attempt=0、lease 轮换、T06 generation_revision=0 不变；T13 不适用）；failure=确定性配置故障 → PLAN_SNAPSHOT_INVALID 终态原子写（T06 无 ready 半成品、T12 同 failed） | **PASS** | worker --once 步进 + SQL 终态核对 / stale=0 | succeeded_jobs=3 stale_warnings=0 gen_ready=True defer={'id': '2c0e503e-dcbd-486e-8fcc-5ccfc889bd19', 'status': 'queued', 'attempt': '0', 'lease_revision': '14', 'generation_revision': '0'} t13=not-applicable(worker T12 defer) failure=a3=16e00fe7-ce8b-43a0-b7c5-2e35b601ecc1 accept=202 p3=809b2f8a-5c76-4af5-b744-74dc75449097 status=failed job=failed ready_rows=0 detail={"code": "PLAN_SNAPSHOT_INVAL |
