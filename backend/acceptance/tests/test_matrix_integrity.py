...
def test_staged_pending_three_state_and_authored_first_wave():
    ...
    assert len(authored) == 94, len(authored)
    assert len(staged) == 0, len(staged)
    batch1 = {
    ...
    batch7 = {f"SC-R-{i:02d}" for i in range(1,15)}
    assert {s["id"] for s in authored} == batch1 | batch2 | batch3 | batch4 | batch5 | batch6 | batch7
    device = [s for s in authored if s["automation_tier"] == "需真实设备或APP"]
    assert len(device) == 33, len(device)
    for s in device:
