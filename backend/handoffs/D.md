...
## InsightFace 接入轮（2026-09-16/17）：phase-1+2 交付完成，最终代码 SHA `5e52cde`（Oracle 第三轮 PASS-with-notes ACCEPTED）
...
## InsightFace 8010 同机活体冒烟轮（2026-09-17）：真实链路+失败/重试映射实证、零写入；质量重拍门未 live 触发（如实记录）
...
- **零写入证明**：library_revision 0→0；smoke ns 前后均 404（从未创建）；register_http_issued=0；uptime 连续（无重启）；事后独立复核 /ready revision=0 + 鉴权 info 404。服务无全局 namespace/subject 清单端点 ⇒ 以上为可得最强证明（如实标注）。
...
- **未验收边界（更新）**：401→ProviderConfigError 仍单元验证（token 有效未触发）；register/query 的真实登记成功路径仍单元验证+门关 live 实证（AUTO_ENROLL=false，未授权登记）；真人照片 live 验证仍禁止（B 合成样本非真人脸）；阈值未标定/无活体/部署与监控前提等既有现场验收限制不变（见上一节）；**B 将独立复核 SQLite 全库计数（namespaces=0/subjects=0/revision=0）——已具备复核条件，经协调节通报**。
