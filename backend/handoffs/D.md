...
- 授权链：初版（下游一次性 JSON）→修订一（下游 SSE `/internal/v1/ai/responses:stream`）→修订二（外部契约也 SSE）。接口参考资料 README SHA-256=8d30ef3cee14a47327ecb3b15ed139191821333cef242f65fab3903d7b420d5c **本地未检索到**（未申请外部目录）——实现以指令自含规格+部署代码实地只读真值为准（routes.py:108 路由存在；models.py:51-54 事件常量全为 `response.*` 前缀，修正了实现初期的裸名白名单缺陷）。
...
- **真实流式联调：未执行（不可用，如实记录，不阻塞代码完成）**——API Key 仅存 dev.ai-skin 远端 .env（非已授权本地安全路径；零回显纪律不变），且服务仅监听远端 127.0.0.1:7861，本地 Java 进程不可达；未建隧道、未部署、未取 Key 到本地。解锁前提：经授权安全渠道向本地运行环境注入 APP_GIMBAL_AI_BASE_URL/APP_GIMBAL_AI_API_KEY 且网络可达（或监督者明示授权隧道方式）。
