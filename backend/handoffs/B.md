...
### 17.11 追加修复：smoke 配置 UTF-8 读取（提交 `703c553`，生产代码零改动）
**更正 17.10 的"本轮最终交付 SHA"表述**：17.10 记的 `4a5e52d` 是 Oracle 第二十三轮 PASS-with-notes 所绑定的 SHA；其后因根的真实 smoke 报 `isv.SMS_SIGNATURE_ILLEGAL`，又追加了一次有界修复，**当前最终代码 SHA 为 `703c553d605d32c530091d86790bf64e97f9d09b`**。该追加修复**未触碰任何生产代码**（`backend/web-java/src/main/**` diff 为 0 文件），故 `4a5e52d` 上取得的 Oracle 裁定对其余范围**仍然有效**；按任务规则（生产代码零改动、仅 smoke 工具与测试）本项采用**窄范围自审**，未另起 Oracle 轮次。
**根因与修复**：`AliyunSmsLiveSmokeIT.loadFileProperties()` 曾用 `Properties.load(InputStream)`，按 `java.util.Properties` 规范以 **ISO-8859-1** 解码 ⇒ UTF-8 中文 `app.sms.aliyun.sign-name`（与中文模板参数名）乱码；`AliyunSmsSendGateway:72` 又原样交给 SDK `.setSignName(...)`（网关与 `AliyunSmsProperties:43` 均不做字符集处理）⇒ 阿里云判签名非法。改为 `InputStreamReader(..., StandardCharsets.UTF_8)` + `Properties.load(Reader)`，并抽出包级纯函数缝 `readPropertiesFile(String)`；unicode 转义与反斜杠行连接语义不变；三重 opt-in 与 `SmsMasking` 脱敏**未改动**（自审核实：`requireExplicitOptIn():58`、三处 `Assumptions.abort():60/63/66`、三个门键 `:50-52`、`maskPhone():90/98`、`"OK".equals(result.code()):99` 均原样）。
**验证（orchestrator 亲跑）**：`test-compile` rc=0；短信定向 **36/0/0**（8 类，含显式纳入 `AliyunSmsLiveSmokeIT`，其 surefire XML 为 `tests="0"`＝在 `@BeforeAll` 第一道门容器级中止）；全量 `mvn -B test` **590 run / 0 failures / 0 errors**，BUILD SUCCESS（基线 587+3）。**未发送任何真实短信**的直接证据：两份日志中 `[live-smoke]`（发送前唯一 println）出现 **0** 次、opt-in 标志 **0** 次、`dysmsapi|aliyuncs.com` **0** 次；测试内 `new Client(`/`sendSms(` **0** 次，`AliyunSmsSecurityTest` 的 2 条网关日志用**桩化 caller lambda**（手机号掩码 `+861****0000`）。新增 `AliyunSmsSmokeConfigEncodingTest`(3) 含**判别力证明**：对同样文件字节，旧 ISO-8859-1 读法 `isNotEqualTo(SIGN_NAME)`、整文件按 ISO-8859-1 解码 `doesNotContain(SIGN_NAME)` ⇒ 断言非恒真。测试只用 `@TempDir` 合成文件与占位值，**不读**根的私有 `application-local.properties`。
**生产侧同源风险（须集成方注意）**：Spring Boot 3.5.16 的 `OriginTrackedPropertiesLoader$CharacterReader` 经**字节码核实**为 `new InputStreamReader(in, StandardCharsets.ISO_8859_1)` ⇒ `.properties` 配置源中的中文同样乱码。三种安全方式：改用 **YAML**、用 **unicode 转义序列**、或用**环境变量/系统属性**注入。B 的生产代码本身字符集干净（main/sms 内唯一 `Charset` 用途是 `AliyunSmsCodeProvider:194-195` 的 UTF_8 常量时间比较）。详见 `B-sms-aliyun.md` 第 8 节。
**真实投递证据（根侧提供、B 未独立验证）**：根报告收到真实短信中的 6 位验证码（**验证码与手机号均不记录**）。若该短信来自 `703c553` 之后的 smoke，则说明签名乱码根因已解决、平台已受理并真实下发；执行版本与 `Code`/`RequestId`/`BizId` 原文**以根侧记录为准**。**f02 真实登录仍未由 B 验证**：challenge 为进程内 `ConcurrentHashMap`（`:95`，schema 无对应表，重启即失效、多实例不共享），核销需 `challengeId`（`AuthController:106`），而 B 侧核实无任何 B 实例在运行 ⇒ 该码在 B 侧不可核销。详见 `B-sms-aliyun.md` 第 9 节。
