...
## shuiguang 真实适配器增量：三轮 Oracle 实际调用记录（2026-09-18）

- 审查者：oracle 子代理会话 `ses_f564d4b4bffenySjytDhUhIInb`（与 V3/InsightFace 各轮同一会话；只读、探针亲跑、未重跑套件）。
- **R1 @`9d8aba1b34a2136cd21053294efc28353be98892`：FAIL**（2 BLOCKER+2 IMPORTANT+1 SUGGESTION，均探针实证）：B1 非崩溃永久孤儿敏感照片（最终 attempt 耗尽无清理/照片版本取代 stale 出口在适配器前/部分暂存临时残留）；B2 暂存异常泄漏配置根+task 路径（预建目录为文件→FileExistsError 全文含路径，存活于 JobFailed 链 __context__）；I3 权限随 umask（0775/0664 可被他人读）；I4 DConfig.__repr__ 泄漏新配置值；S5 碰撞绝对化措辞。COMPLIANT：合同保真（恰三图/相对路径/mirrored:false/成功键集严校/9-6-9 原样/无 scores 包裹/零转换）、激活 fail-closed、V3-only、失败分类矩阵。另核出 AdminReportCommandService 引证不存在（事实更正）。
- **R2 @`367bd148f299bea1fefba59e3abbdb98e83632ab`：FAIL**：B2/I3/I4/S5 **CLOSED 确认**（链泄漏 cause/context=None；umask000→0700/0600；repr sentinel 无）；零回归确认。**关键探针 BLOCK**：推断式清理所有权不安全且具破坏性——真实取代下 old(A) remains=true / current(B) deleted=true（删新活跃 job 暂存）；abort=租约丢失风险；最终 attempt 回调在 fenced 提交前；stale 测试仅 revision-only；全版本遍历禁止。新 IMPORTANT：清理失败静默（ignore_errors+弃结果→告警永不触发）。指定修复：仅清退出 job 拥有的不可变 task_id 且所有权安全终态后，**否则交给激活阻断的 ops 机制**。
- **R3 @`18bf17e6041037a6714f846514c96da7d95ae370`：PASS-with-notes，ACCEPTED 为最终 shuiguang 增量代码 SHA**：移除完整性 COMPLIANT（handler 纯删 52 行零插入、签名精确还原、残留 grep 零）；focus-6 明裁**"round-2 BLOCKER 的可接受解法"**（真实 A/B 取代测试判别于被拒实现；孤儿延后=此前要求的安全替代；激活必须保持被 ops 机制阻断）；孤儿披露 COMPLIANT；IMPORTANT（告警可观测）CLOSED；SUGGESTION CLOSED；零回归全确认；blockingFindings=[]；11 探针。**新 IMPORTANT（非阻塞于本不可激活增量，激活门前必须处置或显式接受）**：同内容并发下自清理与他方 _stage 的本地窗口（实证消毒 ShuiguangStagingError；远端 queue 安全）——激活前三选一（跨进程按 task 串行化/staging 目录消失本地可恢复/自清理亦延后 ops 机制）+确定性交错测试+docstring"所有权安全"限定为远端 queue 安全。接受不授权真实激活。
- 证据充分性（审查者原话要点）：绑定/范围/冻结面/移除 grep/控制流全读独立核验；真实取代判别测试与破坏性探针重做；前轮消毒/权限/repr 探针重跑；377 定向绿未重跑但被判定与变更相称。orchestrator 侧每候选亲跑定向绑定（367/376/377 全 exit 0）。
- shuiguang 谱系累计终态：**18bf17e PASS-with-notes（FINAL，不可激活态；激活前提五项见 D.md）**。

## shuiguang 纠偏轮：Oracle 第四轮实际调用记录（2026-09-18）

- 审查者：oracle 子代理会话 `ses_f564d4b4bffenySjytDhUhIInb`（四轮同一会话；只读、探针亲跑、未重跑套件）。本轮按监督者指令**先呈纠偏事实为绑定真值**（shuiguang-test@d7c84c18 真实源：每 POST 新 queue_id；safe_inputs 先于幂等判断执行期读图；单结果删除饿死兄弟条目→18bf17e 自清理论证为假、旧 stub 不忠）。
- **第四轮 @`f57c2cd50514010a7198dc974faa500bd6761c90`：PASS**，**ACCEPTED 为新最终 shuiguang 增量代码 SHA（取代 18bf17e）**。绑定全过（恰 2 文件 +109/−138；handler 零 diff；冻结面全空；零变异残留）。六焦点全 COMPLIANT：移除完整性（analyze()=preflight→stage→submit→poll→result 直路；_cleanup_staged/_remove_staged/rmtree/清理日志与 import 全灭；仅存 :221 本次自有临时 unlink）；纠正语义入档（:1-43 每 POST 新 queue/safe_inputs 时序/下游幂等未核实不得依赖/四残留类/无 resume 或共享 queue 主张）；stub 保真（每 POST 新 q-N :218-232+重复 POST 异 id 断言 :540-552）；兄弟条目判别（:234-252 成功前校验全部挂载路径、缺→INVALID_INPUT；:555-573 第一完成目录仍在+第二成功+missing_paths==[]+异 queue；重引入成功清理必红=判别成立）；零回归（请求/响应严校、9/6/9 保真、分类矩阵、V3-only、激活、消毒、权限、严校器全不变；**移除全部结果删除彻底化解第三轮竞态**）；诚实（18bf17e 论证 superseded 明示；stub 级证据/回退边界/无联调主张如实在）。blockingFindings=[]；**零新发现（全严重级）**；15 探针全过（兄弟饥饿阻断/重复 POST 异 queue/删除零残留/五出口统一保留/同内容并发暂存安全/临时 O_EXCL 自有/前次目标保留/9-6-9+null 不变/handler 零 delta/泄漏纪律不变）。明裁：纠偏事实在实现与测试中完全反映；**真实激活禁止直至四类暂存保留生命周期获经授权的 durable 清理机制**。
- **shuiguang 谱系累计终态**：9d8aba1 FAIL（2B+2I+1S）→367bd14 FAIL（关键探针 BLOCK：推断清理破坏性）→18bf17e PASS-with-notes（历史接受；其保留自清理的幂等论证后被真实源证伪，SUPERSEDED）→**f57c2cd PASS（FINAL，零发现）**。
