...
### 4.19 第十九轮：`local` profile 加载条件解耦（用户授权，基线 `e2148e1`）

**授权范围**：修复"IDEA 仅启用 `local` profile 时报 `SessionProvider` 缺失"；把**环境 profile** 与 **`app.providers.mode` 的实现选择**解耦；**全面核对**所有相关加载条件（点名 `TestDoubleProvidersConfig`、`IdentityProvidersConfig`、`DeviceProofDoublesConfig`），**不能只补 `SessionProvider`**；`local`+doubles 完整加载登录/身份/设备依赖；`prod` profile 或 `app.env=production` 时替身 fail-closed（**含矛盾配置**）并保留既有防护；仅改必要 Java 加载条件与针对性测试；不实现真实算法/OSS、不改业务契约；**不得改/读/提交** `application.yml`（外层 master 同名文件有用户未提交修改）与 `application-local.properties`（含 PG/OSS 秘密）。

**根因取证（orchestrator 亲自完成，详见 `B.md` §16.1）**：`application.yml:10-12` 的 `spring.profiles.default: dev` 在显式启用 `local` 时不生效 ⇒ 生效 profiles 只有 `{local}`；全仓 **10 处 profile 耦合**把实现选择绑在 profile 名上（`TestDoubleProvidersConfig:30` 全限定 `@Profile`、`DeviceProofDoublesConfig:14`、`IdentityProvidersConfig:19`、`OpenApiDocsConfig:41` + 5 个 catalog、`MemberBindingFaceDouble:30` 经 `CareDevTestCondition:38` 第 3 条）；8 个端口类型均被非 config 单例构造注入 ⇒ 上下文必然启动失败。而 `application.yml:72` 的 base 默认本就是 `app.providers.mode: ${APP_PROVIDERS_MODE:doubles}`（与 profile 无关）、`prod` 段才设 `real` ⇒ **纯 Java 加载条件即可解耦，无需改任何公共默认配置**。

**我的取证缺陷（如实记录）**：首次盘点用 `grep '@Profile'` 只得 9 处，并据此**错误断定**"`TestDoubleProvidersConfig` 本来就没有 `@Profile`、不该因 `local` 失配"——实际它写作**全限定** `@org.springframework.context.annotation.Profile(...)`，该模式匹配不到。改用覆盖全限定写法的模式重做后才得到完整 10 处并定位真正根因。这与本轮之前"`grep ErrorCode.X` 漏静态导入"是**同一类错误** ⇒ 教训：盘点"某类注解/调用的全集"前，必须先确认模式覆盖全限定名、静态导入、别名等等价写法，否则会把"漏检"误当"不存在"。

**设计**：新增 `config/NonProductionCondition`（判据恰两条：`app.env` 非 `production` 且生效 profiles 不含 `prod`/`production`；**不要求** dev/test）；用它**等价替换** 10 处 profile 耦合，**不新增也不删除任何 mode 条件**（`IdentityProvidersConfig` 刻意**不加** mode 门，因今天 dev profile + `mode=real` 时该替身同样装配，属既有语义、不在授权内，已列观察项）；`CareDevTestCondition` 改为**委托**新条件前两条并保留第 3 条，其既有测试 **`CareDevTestConditionTest` 必须原样通过、不得改一行**（作为 C 域语义零变化的可验证证明）；五处既有守卫（`ProductionFailClosedValidator`、`DeviceProofFailClosedValidator`、`CareFaceVerifierProductionGuard`、`DocsProductionGuard`、`TestDoubleProvidersConfig.requireNoProductionSignals`）**原样保留**。
**实施前即预见的测试机制变化**：`StorageFailModeProductionFailClosedTest.productionSignalRefusesAssemblyBeforeAnyStoragePortExists`（约 :116-140）依赖"混合 `prod,dev` 下 `@Profile` 因含 `dev` 而匹配、配置照常装配、再由守卫抛 `IllegalStateException`"；解耦后拒装**提前到条件层**（替身根本不被构造，安全属性**增强**），根因变为缺 bean 类异常 ⇒ 须据实改写但**严禁弱化**（仍须断言启动失败、失败源于生产信号下替身未装配、注入开关不可达）。

**实施与验证**：单一实施道 fix-16（文件紧耦合且共享新条件类，不可并行）；orchestrator 独立复核写域与硬约束（`CareDevTestConditionTest` 一行未改、main java 中真实 `@Profile` 注解残留 **0**、四处守卫文件零改动、`src/main/resources` 零改动），并亲跑 `test-compile` 0、定向 46/0/0、全量 **465/0/0**、`local` 与 `prod` 真实进程对照 ⇒ 提交 `39608bf`。
**送审范围（拟）**：请特别复核 ①五处既有守卫是否全部原样保留且未被绕过（尤其混合 `prod,dev` + `app.env=dev` + `mode=doubles`）；②`IdentityProvidersConfig` 未新增 mode 门是否与既有语义等价；③docs 6 类去掉 profile 门后，生产 fail-closed 是否仍完全由 `DocsProductionGuard` + `springdoc.*` 属性承担；④`CareDevTestConditionTest` 原样通过是否足以证明 C 域语义零变化；⑤`local` profile 真实进程证据是否足以证明用户报障已修复（我方无法读取用户私有 `application-local.properties`，只能以显式 env 覆盖模拟"仅 `local` profile"，此局限须如实披露）。

**第十九轮（Oracle 对 `39608bf` 的有界复审）结论：VERDICT PASS-with-notes**——八项判据全部通过（解耦完整正确；生产拒装成立且条件层比旧 `acceptsProfiles("prod")` 覆盖更广；测试改写未弱化；C 域改造可接受、trim 差异属 fail-closed 增强；docs 防护成立；`IdentityProvidersConfig` 不加 mode 门符合最小等价替换；真实进程证据足以证明报障已修复、无需修复前对照），**许可把 `39608bf` 交总协调整合**、不要求补跑完整 41 项或 E 全套、确认轮次边界。新发现 2 IMPORTANT（① 两个测试提供者缺 mode 门，`mode=real` 下仍装配且护理替身 `@Primary` 可能压过未来真实实现；② `DocsProductionGuard` 不识别字面 `production` profile）+ 3 SUGGESTION；并裁定"`requireNoProductionSignals` 保留可接受，但它**不是**真正的纵深防御——其 profile 判断反而比共享条件弱，注释'被绕过时兜底'略夸大"。

### 4.20 第二十轮：总协调在飞增量（提交 `6a72b0b6cf2efb80eb6f641124dd4b434328f0c6`）：**PASS-with-notes**

增量两项与 Oracle 两项 IMPORTANT 重合，合并为一轮实施（12 文件 / +675 −128，业务代码 diff 0）：①`IdentityProvidersConfig` 与 `MemberBindingFaceDouble` 补 `@ConditionalOnProperty(app.providers.mode=doubles, matchIfMissing=true)`；②新增 `config/ProvidersModeProductionGuard`（`BeanFactoryPostProcessor`，早于常规单例实例化）使"生产信号 + `doubles`"得到明确诊断而非被"缺 `SessionProvider`"掩盖，并据实改写 `StorageFailModeProductionFailClosedTest` 4 个生产信号用例（保留"替身从未构造、注入开关不可达"两条强断言）；③`DocsProductionGuard` 判据复用 `NonProductionCondition`（字面 `production` 不再漏判）+ 新增回归用例，`requireNoProductionSignals` 复用同一判据并修正夸大 javadoc；④**orchestrator 自行发现并补齐的既有缺口**——`mode=disabled` 此前**无法启动完整应用**（B 自有 `PairingProofVerifier`/`ConnectionProofVerifier`/`FaceIdentityResolver` 均无 disabled 占位，A 的 `DisabledProvidersConfig` 只覆盖 5 个 A 端口；实测失败于 `GimbalBindingService` 缺 `PairingProofVerifier`），故新增 `DisabledDeviceProofVerifiersConfig` 与 `DisabledFaceIdentityResolverConfig`（`mode=disabled` 专用、**刻意不加环境门**以与 A 对称、由 mode 与替身配置互斥），一律抛 `503 DEPENDENCY_UNAVAILABLE`，**绝不**返回"证明有效/可靠匹配"。
另如实记录：orchestrator 修正了自己 brief 的一处设计错误（原要求给 `DisabledFaceIdentityResolverConfig` 加环境门，会造成"`prod` profile + `app.env=dev` + `disabled`"下缺 bean 并掩盖真因）；实施道一处表述不准确（`prodProfileWithDisabledPassesGuard` 的"允许启动"在隔离 runner 中成立、在完整应用中当时并不成立——补齐占位后该措辞才变为真实）。
**验证**：`test-compile` 0；定向 **61/0/0**（含 `CareDevTestConditionTest` 1、文件一行未改）；全量 **481 run / 0 failures / 0 errors**（基线 465 + 16）。真实进程四例（18083 + 55435 的 `mvp_b_dev`，用毕 kill、端口已释放）：`local` → UP + health 200 + 无 token 得 **401 `AUTH_REQUIRED`**（`SessionProvider` 已装配）；`prod`(yml mode=real) → 失败缺 `SessionProvider`（既有刻意 fail-closed）；`prod`+`APP_PROVIDERS_MODE=doubles` → 失败且根因为**新守卫诊断**、`SessionProvider` 缺失计数 **0**；`local`+`--app.providers.mode=disabled` → **UP + health 200 + `sms-challenges` 503 `DEPENDENCY_UNAVAILABLE`**。
**Oracle 第二十轮判定**：增量 1–4 **全部正确**；第 4 项**属合理最小范围**（"抛 503 比返回 `Result.failed()` 正确，后者会被解释为 400/403 类证明失败而非能力关闭"）；**可以把 `6a72b0b` 交由总协调整合**；**不要求补跑验证**；**未发现真正阻塞项**；未改变业务契约或 HTTP 语义（唯一新增可观察行为是"此前无法启动的 `mode=disabled` 现在可启动并让相关能力返回既有 503"，属授权内配置语义修复）。新发现仅 2 SUGGESTION（非阻塞）：①`DeviceProofFailClosedValidator.isDouble()` 只识别类名以 `DevTestDouble` 开头，故 lambda 形式的 disabled 占位不被它识别为"非真实实现"（当前仍由 A 的 `ProductionFailClosedValidator` 拒绝全局 `disabled`，启动安全未受损）；②新守卫诊断文案把 `disabled` 列为生产信号下的替代方案，但 `app.env=production` 时 A 的 validator 仍会拒绝完整应用，宜注明 `disabled` 仅用于明确允许的非生产/降级环境。两项按"不为非阻塞措辞循环审查"的裁定**不改代码 SHA**，记此待后续硬化任务处理。

---

### 4.21 第二十一轮：真实供应商接入（短信 / OSS / 独立人脸服务）

**授权范围**：接真实阿里云短信、Java+Python 真实阿里云 OSS、dev.ai-skin 上完全独立的 `InsightFace-for-openvela`；按服务独立选择；真实服务故障不得回退 mock 成功；短信受理成功必须 `Code=OK`、发送失败不签发有效 challenge、未获测试手机号前不发真实短信；人脸缺 1:1/可靠新人/活体必须明确未接入并拒绝或不确定，不得伪完成；不读取外层私有配置、不把凭据写入会话或产物。

**提交链**：`0cc9f76`（人脸缺口与最小接口方案，用户据此选定 A 方案并授权部署）→ `07254a0`（短信）→ `229476c`（Python OSS）→ `dce562b`+`13f0f62`（人脸服务 + EOF 修正）→ `20e2575`（Java OSS）→ 在飞（Java 人脸 adapter）。事实、验证数字、部署实测证据与我自己 8 项错误见 `B.md` §17.1–17.5。

**送审范围（拟）**：①短信 `Code=="OK"` 唯一成功判据与"失败不签发 challenge"的构造性保证、三窗口 UTC+8 节流、`123456` 仅 doubles、脱敏；②Java/Python 双侧 OSS 的私有桶语义（无 `generatePresignedUrl`、后端流式转发）、失败分类与重试语义（保留 `RESULT_ARCHIVE_FAILED` 可重试）、objectKey 跨语言一致、Java 侧 bucket 一致性启动校验；③人脸服务的零写入 extract、严格 1:1、命名空间隔离与回执、活体恒 `supported:false`、用户级 unit 与 linger 前提；④Java 人脸 adapter 的三条红线（`classify` 永不 MATCHED/RELIABLE_NEW、insightface 下不以 sha256 冒充身份、`aliyun` 仅边界且选中即拒绝启动）；⑤各服务独立选择与既有 `app.providers.mode`/生产守卫的优先级关系是否未被绕过。请特别复核：真实故障是否存在任何静默回退替身成功的路径；凭据/手机号/验证码/token 是否在任何日志、异常消息、`toString()`、文档中出现；以及"未接入"项是否都被诚实标注而非以替身行为冒充。

**第二十一轮（Oracle 对 `55f666e` 的聚焦复审）结论：VERDICT FAIL**——4 BLOCKER + 5 IMPORTANT + 1 SUGGESTION，**明确不可整合**。同时它确认了大量正面结论：①**未发现任何"真实失败后回退替身成功"的路径**（短信 `AliyunSmsSendGateway:99-128`、Java OSS `OssStorageAdapter:58-132`、Python OSS `storage.py:146-211`、人脸 `InsightFaceProvider:40-89`）；②短信生命周期顺序正确（发送成功后才写 challenge `:78-100`、`SecureRandom` 且排除 `123456` `:162-172`、一次性/TTL/尝试上限 `:105-124`、UTC+8 自然窗口 `:192-223`）；③OSS 主体语义正确（无 presigned/公开 URL、objectKey 两侧一致、**Java bucket 一致性启动校验是正确的最小做法**、`NoSuchKey` 与配置/依赖分类正确）；④人脸服务的零写入（`api.py:273-305`）、严格 1:1（`:335-385`）、命名空间隔离（`store.py:137-282` 每条 SQL 均带 namespace）、回执不返回 embedding（`api.py:387-477`）、延迟导入（`model.py:113-130`）**全部成立**；⑤Java adapter 三条红线中两条成立（classify 永不 MATCHED/RELIABLE_NEW；insightface 下不用 sha256 冒充身份；aliyun 仅早期拒绝边界）；⑥生产处置正确且**未绕过 C 的守卫**；⑦装配优先级正确、`@ConditionalOnProperty` 重复标注在 Boot 3.5 具 AND 语义（我已另用 `javap -v` 在字节码层面证实该注解带 `Ljava/lang/annotation/Repeatable;`）；⑧仓内未发现真实凭据或真实手机号。

**orchestrator 对每条发现的独立读码核实（不盲信、不盲驳）——全部成立**：
- **BLOCKER 1（护理 1:1 不要求活体）**：`FaceServiceClient.verify`(`:84-95`) 只发 `image`/`namespace`/`subject_id`/可选 `threshold`，**整个 Java face 包中 `require_liveness` 出现 0 次**；`InsightFaceCareVerifier:50` 直接 `result.matched() ? MATCHED : MISMATCH` ⇒ **活体未接入却可通过护理准入**，正是用户明令禁止的"伪完成"。服务端其实已支持该门（`api.py:339` 调 `_ensure_liveness_supported`，为真即 501），且 `InsightFaceCareVerifier:53-55` **已**把 `LIVENESS_UNSUPPORTED` 映射为 `CAPABILITY_UNAVAILABLE` ⇒ 修法很小但**后果重大**：修后 insightface 模式下护理准入在活体落地前**恒 503 不可用**（刻意 fail-closed，必须向根明确上报为功能边界而非缺陷）。
- **BLOCKER 2（服务鉴权默认 fail-open）**：`config.py:91` `auth_required=False`、`:128` 默认 False，`_env_bool`(`:54-58`) 对任何未知值**静默返回 False**；默认 host 是**非 loopback 的 `10.3.6.163`**(`:65`,`:115`) ⇒ 默认配置＝内网暴露且鉴权关闭。我已部署的实例因 `.env.example` 带 `AUTH_REQUIRED=true` 而受保护，但**默认值本身不安全**。
- **BLOCKER 3（短信节流 check-then-send 竞态）**：`issue()` 的预检(`:79`)→发送(`:87`)→记录(`:97`) **无跨三者的锁**（`:110/:181/:200` 的 `synchronized` 均为局部）⇒ 同手机号并发可全部通过预检并各发一条**计费**短信，突破"1 分钟 1 条"。
- **BLOCKER 4（Python OSS 配置错误被当可重试）**：`storage.py:199-209` 中 `StorageConfigError` 仅出现在 `:205/:207`，`NoSuchBucket` 落入 `:209` 的可重试兜底 ⇒ 永久性配置错误会无限重试；且源图读取路径吞掉 `StorageConfigError`。
- **IMPORTANT 5（access log 泄漏身份引用）**：`main.py:37` `access_log=True` + `api.py:438-459` 路由含 namespace/subject_id ⇒ 身份引用进 journal。**这是我的核验盲区**：我此前的 journal 检查只 grep 了 error 行与 token 泄漏，**没有检查身份引用**。
- **IMPORTANT 6（源图读取不校验 bucket）**：`dmedia.py:123-124` 的 `_SELECT_SOURCE_MEDIA` 只选 `id, object_key, state`，**不读 `bucket`** ⇒ Java 写 A 桶、Worker 配 B 桶时会静默读错桶。
- **IMPORTANT 7（启动诊断回显真实桶名）**：`OssProvidersConfig:53-56` 拼接两个真实桶名。**这是我上一轮的错误决定**——我在任务书里明确写了"消息可输出 bucket 名，它不是凭据"。Oracle 按本轮日志卫生标准判为 IMPORTANT，**我采纳并纠正自己的决定**（改为只列键名 + values omitted）。
- **IMPORTANT 8（短信内存态无界）**：`challenges`/`acceptedSends` 为无界 map，仅在核销(`:115`)与耗尽(`:129`)时 remove，过期未核销者永不回收、phone key 永不删除。
- **SUGGESTION 9（verify 响应宽松解析）**：`FaceServiceClient:99-103` 用 `asBoolean(false)`/`asDouble(NaN)` 静默降级，stub 只返回 `{"matched":true}` 也会被接受为 MATCHED——与 BLOCKER 1 同源，一并修。

**处置**：按**构建工具互斥**分三道并行（写域零重叠）——Java 道独占 mvn（BLOCKER 1/3 + IMPORTANT 7/8 + SUGGESTION 9）、face-service 道独占其自有 venv（BLOCKER 2 + IMPORTANT 5）、worker-python 道独占其 venv（BLOCKER 4 + IMPORTANT 6）。BLOCKER 1 的修法确定为**恒定发送 `require_liveness=true` 且不提供关闭开关**（设置"可削弱安全"的开关不可接受）。Oracle 对第 6 条的裁定一并记录：(a) 生产回落 `FailClosedCareFaceVerifier` 的处置**正确**、未绕过 C 的守卫；(b) 生产启用**必须由总协调授权**修改 `CareFaceVerifierProductionGuard`，且应以明确的"可信且生产就绪"marker 放行、而非按类名放行，**前提是活体问题先闭合**；(c) 理论上可先合入"非生产真实 provider"，但因本轮其它 blocker 故 `55f666e` 不可整合。

**第二十二轮（Oracle 对修复后最终 SHA 的窄范围复审）结论**：待填（按其轮次边界声明，下一轮只需核验四类阻塞修复及相应定向测试，无需重审其它已通过内容）。

### 4.22 第二十二轮：四类阻塞整改与我自行发现的部署缺陷（提交 `2d9ee97f7093e2b340562cc5d3e215f43b7d7ea6`）

修复链：`ffa4570`（face-service：BLOCKER 2 + IMPORTANT 5）→ `11ff41c`（Java：BLOCKER 1/3 + IMPORTANT 7/8 + SUGGESTION 9）→ `f75b5be`（Python：BLOCKER 4 + IMPORTANT 6）→ `2d9ee97`（**orchestrator 自行发现的 `deploy.sh` 缺陷**：`enable --now` 不重启已运行 unit ⇒ 重新部署静默保留旧代码；改为 `enable` + 无条件 `restart` + `ExecMainPID` 变更断言）。逐项事实、验证数字与决定性实测见 `B.md` §17.7。
送审时请 Oracle 裁定的五个具体问题：①BLOCKER 1 的"2xx 但 `supported=false` 即抛 501"额外加固是否恰当、`classify`/`extract` 刻意不加活体门是否可接受；②BLOCKER 3 把远端发送放在锁内是否可接受、64 条带是否足够；③BLOCKER 4 的**兜底方向选终态**是否正确；④`deploy.sh` 修法是否充分（是否还应校验运行中代码的版本/构建标识）；⑤两项写域外残留（`identity_enroll.py` 的配置错误经 `loop.py` 有界重试而非即时终态；短信容量上限仅构造器可注入且跨条带允许有界瞬时超出）可否接受或须另开任务。
**第二十二轮（Oracle 对 `2d9ee97` 的窄范围重绑定复审）结论**：待填。

### 4.23 第二十二轮 Oracle 判 FAIL：四类阻塞确认闭合，新 BLOCKER 我方读码异议与"用测试裁决"处置

Oracle 对 `2d9ee97` 判 **VERDICT: FAIL**，但范围极窄且**正面结论为主**：
- **四类阻塞全部确认闭合**：①护理 1:1 活体——恒发 `require_liveness=true`(`FaceServiceClient.java:105-119`)、2xx 但 `supported=false` 也拒绝(`:142-165`)、严格响应解析(`:123-180`)；并裁定 (b) 额外校验**正确**、(c) `extract`/`classify` 不加活体门**可接受**（因 classify 永不产生 MATCHED/RELIABLE_NEW、M1 身份解析仍 Unavailable）。②Face Service 鉴权与日志——严格布尔(`config.py:56-74`)、auth 默认 true 与 non-loopback 联动(`:119,156,193-215`)、`create_app` 纵深校验(`app.py:37-46`)、原始 uvicorn access log 关闭(`main.py:22-27`)、路由模板脱敏日志(`app.py:90-137`)；**未发现未授权请求可触达 extract/verify/register/delete 的路径**。③短信签发节流竞态——64 条带锁覆盖预检→远程发送→记录→challenge 写入(`:127-168`)；**同号远程调用置于锁内是正确取舍**，64 条带足够。④Python OSS 配置错误——`NoSuchKey` 仅映射不存在、`NoSuchBucket`/权限/签名等终态(`storage.py:154-282`)、**未知无 status 异常兜底终态是合理 fail-closed**、source 读取不再吞 `StorageConfigError`(`dmedia.py:499-530`)、bucket 读取前匹配(`:515-528`)、归档瞬态重试语义未弱化。
- **随附修复全部判有效**（bucket 值已隐藏、verify 不再以缺省值伪造 MATCH/MISMATCH、challenge 与 phone 跟踪有上限、access 日志不含 namespace/subjectId 原值）。
- **`deploy.sh` 修法判充分**：`enable` 后无条件 restart(`:140-150`)、已运行服务要求 PID 变化(`:151-163`)、后续 health 轮询(`:165-183`)，"对『部署成功但旧进程仍服务』已形成有效判别"；构建 SHA/version 进入 health 属**可选增强、不是阻塞**。
- **两项残留披露均判可接受为非阻塞待办**：①`identity_enroll` 的配置错误有限重试——"不会回退成功，也不会无限重试"，应由 **D handler 所有者**增加 `StorageConfigError → retryable=false` 定向映射，**不应为此修改公共 runtime**；②短信容量仅构造器可配、跨条带瞬时超限——默认 10000 且 challenge 预留严格有界，tracked-phone 最多按并发签发量短暂超限、不会无界增长，后续应经 `SmsRiskProperties` 暴露两个上限但**不阻塞本轮**。
- **回退与泄漏结论**：未发现真实服务失败静默回退 double 或伪造成功；未发现提交中有真实 AK/SK/token/验证码/完整手机号/embedding；Face Service 原始路径 PII 日志已闭合；Python OSS 日志仍暴露真实 bucket/objectKey，属**需整改的日志卫生问题、不是凭据泄漏**。

**新发现三项**（Oracle）：BLOCKER＝`AliyunSmsCodeProvider.java:175-204` challenge 一次性核销为 check-then-set，两个并发正确码请求都可能在 `consumed=true` 前通过检查并各自返回手机号 ⇒ 同一 challenge 签发两个会话；IMPORTANT＝`storage.py:233-243` OSS 错误日志仍输出真实 bucket 与完整 objectKey（与已按要求隐藏的 Java 侧不一致）；IMPORTANT＝`FaceServiceClient.java:142-165` 将来仅凭 `liveness.supported=true` 就会允许 matched，尚未定义**本次样本**的 `passed/result`。

**orchestrator 的独立读码核实**：两项 IMPORTANT **成立**（`storage.py:233-243` 确实写 `bucket=bucket` 与 `objectKey=object_key`，而 Java 侧 `OssStorageAdapter:125/132/142` 只写 op/code/requestId；`:142-156` 只校验能力位 `supported`，`supported=true` 时径直走到 `:165` 返回 `VerifyResult(matched,…)`，服务端今天恒返回 `{"supported":false,…}`(`api.py:270,303,331`) 故当前不可达）。**但新 BLOCKER 的竞态前提我方读码认为不成立**：`verify()` 的 `:180` 已是 `synchronized (challenge)`，而 `:181` 的 `consumed` 检查、`:184` 过期检查、`:188` 尝试上限、`:193-195` 常量时间比较、`:197` attempt 递增、`:199`/`:204` remove、`:203` 置位、`:205` 返回手机号**全部在该 monitor 内**；`:379` `Challenge` 为 `private static final class`、`:384` `consumed` 为 `volatile`；`challenges` 是 `ConcurrentHashMap`，同一 id 的 `get` 必返回**同一实例** ⇒ 并发请求在该 monitor 上串行，第二个见 `consumed=true` 即 `Optional.empty()`；唯一返回手机号的 `:205` 在锁内且在置位之后。可达性确认（`auth/AuthController.java:106` 用返回值作 `loginSubject` 签发会话）⇒ **后果描述成立，但竞态前提疑似不成立**。

**处置原则（重要）**：我方**不以读码结论对抗审查**，也**不盲从**——Oracle 要求的并发核销测试本就**缺失**，而"看起来正确"的推理本轮已多次被事实推翻（含我自己的四次工具/环境缺陷）。故采用**用测试裁决**：要求实施道**先只写测试、不改生产代码**（N≥16 线程 + `CyclicBarrier` 同时提交正确码、断言恰一成功；再加 200 轮 × 8 线程带 jitter 的多轮版本以排除运气；再覆盖混合正确/错误码时"尝试耗尽后仍能成功核销"不得发生），然后：测试通过 ⇒ **不改生产代码**（盲目改动正确的同步有害）、以真实数字与锁覆盖范围回报；测试失败 ⇒ Oracle 正确，立即修复并保留**修复前失败输出**作证据。无论结果，**新增并发测试都留在仓库**作为长期回归守卫。
两项 IMPORTANT 一并修（均为小改动、且 IMPORTANT 2 现在闭合可避免将来再开一轮）：Python 日志改为只写 `bucketConfigured`、**用途段**（objectKey 第二段 purpose）、operation/ossCode/httpStatus/requestId，**绝不**写桶名、完整 key 或 media_id；Java 侧冻结活体契约——`supported=true` 时**强制要求**本次样本的 `liveness.passed` 存在、为 JSON boolean 且为 `true`，缺失/非 boolean/为 false 一律拒绝（新码 `LIVENESS_NOT_PASSED`，区别于 `LIVENESS_UNSUPPORTED`，保守映射 501），**不得**把"能力支持"当作"样本通过"，**不得**引入任何跳过活体校验的开关；并注明活体真正落地时该映射须重新评估。
**第二十三轮（Oracle 对修复后最终 SHA 的窄范围复审）结论**：待填（按其边界声明，下一轮只需核验短信 challenge 并发核销的测试裁决结果及对应测试，其它已闭合内容无需重审；本轮我方另修两项 IMPORTANT，故一并请其确认）。

### 4.24 第二十三轮：并发核销裁决（BLOCKER 判不成立）与两项 IMPORTANT 修复（提交 `4a5e52d46ab2ca75853d21678216e87891014e1a`）
提交链：`a6f7a50`（Java：`AliyunSmsVerifyConcurrencyTest` 3 项 + `liveness.passed` 契约冻结 + `LIVENESS_NOT_PASSED`→`CAPABILITY_UNAVAILABLE` 映射）→ `4a5e52d`（Python：`storage.py:239-249` 日志卫生）。事实、锁覆盖复核、测试构造与验证数字见 `B.md` §17.9。
送审时请 Oracle **二选一表态**：(A) 撤回该 BLOCKER 并确认"保留正确同步 + 新增永久回归测试"是恰当处置；或 (B) 维持判定但**必须给出能击穿该 monitor 的具体交错序列**（何种路径使两线程持有不同 `Challenge` 实例、或返回手机号的语句在锁外执行、或 `volatile`+monitor 可见性论证何处失效），不得仅以"check-then-set 模式可疑"维持。并请其裁定：①测试强度是否足够（若不足请指出具体弱点与所需更强形态）；②`storage.py:140`/`:118` 保留含完整 objectKey 的**异常消息**（已证明不进日志与 `last_error`）是否可接受，或须一并脱敏；③`LIVENESS_NOT_PASSED` 保守映射 501 与"活体落地时须重评估"的标注是否恰当。
**第二十三轮（Oracle 对 `4a5e52d` 的窄范围复审）结论：VERDICT PASS-with-notes。**
- **争点一：Oracle 选 (A) 撤回该 BLOCKER**，并明确承认"我上轮遗漏了 `AliyunSmsCodeProvider.verify()` 外层已有的 `synchronized (challenge)`"（`:180-206`）；其复核结论为：同一 `challengeId` 从 `ConcurrentHashMap` 取得同一 `Challenge` 实例，检查、attempt 递增、常量时间比较、`consumed=true`、条件 remove 及**唯一成功 return** 全在同一 monitor 内，第二个线程只能在首个退出后进入并看到 `consumed` 或 map 已无该对象 ⇒ **"不存在能击穿该 monitor 的正常交错序列"**；`volatile` 虽冗余但不削弱 monitor 语义。并裁定新增测试**有效**（32 线程恰一成功、200×8 带调度扰动、正误码混合不产生多次成功、`Future` 异常/超时会使测试失败），以及 **"未改正确生产代码、仅补永久回归测试是恰当处置"**。
- **两项 IMPORTANT 均判闭合**：①Python OSS 日志卫生——`storage.py:239-249` 不再输出 bucket/完整 objectKey/mediaId，只保留 operation/bucketConfigured/purpose/code/status/requestId，`:75-77` 解析失败返回固定 `<unparsed>` 不回退原 key；**`storage.py:118,140` 保留含 key 的内部异常判可接受**（当前调用链会在业务边界改写为固定 reason 或固定 Archive/Job 错误，不进日志、公开响应或 T12 `last_error`），并提示"后续新增调用方仍需遵守该净化边界"。②样本级活体——`FaceServiceClient.java:167-184` 要求 `supported=true` + `passed` 存在且为 JSON boolean 且为 `true`，失败映射 `InsightFaceCareVerifier.java:63-69`，当前真实服务恒 `supported=false` 故护理仍 503 fail-closed；并指出**未来真实活体实现须重新冻结"未通过"应映射 `QUALITY_REJECTED` 还是 `CAPABILITY_UNAVAILABLE`**（当前用 501/503 保守拒绝，不会伪造 MATCHED）。
- **新发现：无。**
- **整合许可**：**可以将 `4a5e52d46ab2ca75853d21678216e87891014e1a` 作为本轮最终交付 SHA 交总协调整合**；**不要求补跑验证**（587 项 Java 全量、352 项 Python 全量及新增并发/日志/活体定向测试已覆盖本增量，无需完整验收或 E 全套）。
- **回退与泄漏结论**：未发现真实故障静默回退替身或伪造成功；未发现 AK/SK/token、验证码、完整手机号、embedding 或身份引用日志泄漏；OSS 内部异常仍可能携带 objectKey 但当前所有调用边界均净化、不对外传播；人脸真实服务尚无活体，护理准入继续明确不可用、不冒充完成。
- **轮次边界确认**：`4a5e52d` 为本轮最终交付 SHA；**无需再因该短信并发争点、日志措辞或测试精度发起重绑定轮次**。持续披露但不阻塞五项：①`identity.enroll` 遇 OSS 配置错误仍有限重试至 `max_attempts`；②短信容量阈值尚未暴露为 Spring 配置；③真实活体落地时须冻结样本失败映射；④生产 `CareFaceVerifier` 守卫尚未授权真实实现；⑤真实短信/OSS/成员人脸正向链路尚未使用真实数据验证。
- 一处 Oracle 命令清单中的文件名笔误（写作 `tests/test_aliyun_oss_storage.py`，实际为 `tests/test_oss_storage_provider.py`）；其结论引用的 `storage.py` 行号与内容均与实仓一致，不影响裁定。
