...
def delete_files_from_db(session, knowledge_base_name: str):
    ...
    session.commit()
    try:
        from rag.connector.database.repository.indexing_diagnostic_repository import delete_indexing_diagnostics
        delete_indexing_diagnostics(knowledge_base_name)
    except Exception:
        # Diagnostic cleanup is best-effort and must not block knowledge-base cleanup.
        pass
    return True
...
def delete_file_from_db(session, kb_file: KnowledgeFile):
    ...
    if existing_file:
        ...
        try:
            from rag.connector.database.repository.indexing_diagnostic_repository import delete_indexing_diagnostics
            delete_indexing_diagnostics(kb_file.kb_name, kb_file.filename)
        except Exception:
            # Diagnostic cleanup is best-effort and must not block re-indexing.
            pass
    return True
